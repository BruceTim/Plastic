...
(function ($) {
    $.fn.extendPagination = function (options) {
        ...
        if (totalPage > 0) {
            var html = [];
            html.push('<div><div class="page-info"><span>Page&nbsp;&nbsp;' + pageNum + '&nbsp;&nbsp;of&nbsp;&nbsp;' + totalPage + '</span></div>');
            ...
            html.push('<li class="disabled hidden more-prev"><a href="javascript:void(0)">...</a></li>');
            if (totalPage <= showPage) {
                for (var i = 1; i <= totalPage; i++) {
                    if (i == pageNum) html.push(' <li class="active"><a href="javascript:void(0)">' + i + '</a></li>');
                    else html.push(' <li><a href="javascript:void(0)">' + i + '</a></li>');
                }
            } else {
                if (pageNum <= middle) {
                	for (var j = 1; j <= showPage; j++) {
                    	if (j == pageNum) html.push(' <li class="active"><a href="javascript:void(0)">' + j + '</a></li>');
                	...
                	for (var k = totalPage - showPage + 1; k <= totalPage; k++) {
                    	if (k == pageNum) html.push(' <li class="active"><a href="javascript:void(0)">' + k + '</a></li>');
                    	else html.push(' <li><a href="javascript:void(0)">' + k + '</a></li>');
                	}
                } else if (pageNum > middle && pageNum < totalPage-(showPage - middle)) {
                	for (var n = pageNum - middle + 1; n < pageNum; n++) {
                    	html.push(' <li><a href="javascript:void(0)">' + n + '</a></li>');
                	}
                	for (var p = pageNum; p <= pageNum + (showPage - middle); p++) {
                		if (p == pageNum) html.push(' <li class="active"><a href="javascript:void(0)">' + p + '</a></li>');
                    	else html.push(' <li><a href="javascript:void(0)">' + p + '</a></li>');
                	}
                }
            }
            html.push('<li class="disabled hidden more-next"><a href="javascript:void(0)">...</a></li>');
            html.push('<li class="next"><a href="javascript:void(0)">&raquo;</a></li>');
            html.push(' <li class="last"><a href="javascript:void(0)">&gt;|</a></li>');
            html.push(' </ul></div></div>');
            $(this).html(html.join(''));
            if (pageNum == 1) {
                $(this).find('ul.pagination li.previous').addClass('disabled');
                $(this).find('ul.pagination li.first').addClass('disabled');
            }
            if (pageNum == totalPage) {
                $(this).find('ul.pagination li.next').addClass('disabled');
                $(this).find('ul.pagination li.last').addClass('disabled');
            }
			if (totalPage > showPage && pageNum > middle) {
				$(this).find('ul.pagination li.more-prev').removeClass('hidden');
			}
			if (totalPage > showPage && pageNum < totalPage - (showPage - middle)) {
				$(this).find('ul.pagination li.more-next').removeClass('hidden');
			}
            var pageObj = $(this).find('ul.pagination'), preObj = pageObj.find('li.previous'),
                currentObj = pageObj.find('li').not('.previous,.disabled,.next'),
                nextObj = pageObj.find('li.next'), firstObj = pageObj.find('li.first'),
                lastObj = pageObj.find('li.last');
